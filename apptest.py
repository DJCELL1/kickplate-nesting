...
def generate_pdf_cutlist(sheets: List[Sheet], stock_width: int, stock_height: int, 
                        grain_direction: str, logo_file=None, filename: str = "cut_list.pdf") -> bytes:
    ...
    if logo_file:
        try:
            ...
            logo_img = Image(logo_file, width=60*mm, height=30*mm, kind='proportional')
    ...
    for sheet in sheets:
        ...
        # Create a table to hold diagram and cutting list side by side
        # Count unique pieces and their quantities
        piece_counts = {}
        for p in sheet.placements:
            key = f"{p.code} ({p.width}×{p.height}mm)"
            piece_counts[key] = piece_counts.get(key, 0) + 1
        
        # Create simplified cutting list
        cutting_list_data = [['Kickplate', 'Qty']]
        for piece, qty in sorted(piece_counts.items()):
            cutting_list_data.append([piece, str(qty)])
        
        cutting_list_table = Table(cutting_list_data, colWidths=[50*mm, 20*mm])
        ...
        diagram_img = Image(img_buffer, width=190*mm, height=127*mm)
        
        # Combine diagram and table side by side
        layout_table = Table([[diagram_img, cutting_list_table]], colWidths=[190*mm, 70*mm])
...
def main():
    ...
    # Initialize session state for logo
    ...
    if st.session_state.logo:
        ...
        with col1:
            st.image(st.session_state.logo, width=120)
        with col2:
    ...
    with st.sidebar:
        ...
        logo_file = st.file_uploader("Upload Logo (PNG/JPG)", type=['png', 'jpg', 'jpeg'], help="Optional: Add your company logo to PDFs and app header")
        
        if logo_file:
            st.image(logo_file, width=150)
            # Store logo in session state
                ...
                if st.button("🚀 Generate Cut List", type="primary", use_container_width=True):
                    ...
                    with col2:
                        pdf_data = generate_pdf_cutlist(sheets, stock_width, stock_height, grain_direction, 
                                                       logo_file=st.session_state.logo if 'logo' in st.session_state else None)
                        st.download_button(
                            label="📄 Download PDF",
                            data=pdf_data,
